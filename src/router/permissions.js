 ...
 * 「系統紀錄」（/log）、「Prompt 模板」（/prompt-text）：僅 user_type=1
 */
...
export function userMayAccessRoute(user, to) {
  ...
  if ((key === 'log' || key === 'design' || key === 'logo' || key === 'prompt-text') && Number(user.user_type) !== DEVELOPER_USER_TYPE) return false;
...
export function canSeeNavLink(userType, viewKey) {
  if (viewKey === 'log' || viewKey === 'design' || viewKey === 'logo' || viewKey === 'prompt-text') return Number(userType) === DEVELOPER_USER_TYPE;
