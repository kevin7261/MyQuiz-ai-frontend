...
export function userMayAccessRoute(user, to) {
  ...
  if ((key === 'logs' || key === 'design' || key === 'design_2' || key === 'logo') && Number(user.user_type) !== DEVELOPER_USER_TYPE) return false;
...
export function canSeeNavLink(userType, viewKey) {
  if (viewKey === 'logs' || viewKey === 'design' || viewKey === 'design_2' || viewKey === 'logo') return Number(userType) === DEVELOPER_USER_TYPE;
